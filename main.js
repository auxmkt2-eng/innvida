...
function renderComparison(){
  ...
  const a=by('SANARE'),b=by('NOMAD'),max=Math.max(a.total,b.total,1);
  $('#brandComparison').innerHTML=[['Sanaré',a],['Nomad',b]]
    .map(([name,m])=>`<div class="brand-row"><strong>${name}</strong><div class="bar"><i style="width:${m.total/max*100}%"></i></div><span class="number">${mxn.format(m.total)}</span></div><div class="small" style="margin:-12px 0 0 100px">Cerrado ${mxn.format(m.closed)} · Estimado ${mxn.format(m.estimate)} · ${m.count} casos</div>`)
    .join('');
}
...
function renderKams(){
  ...
  const kams=Object.entries(groups)
    .map(([name,list])=>({
      name,
      list,
      m:metrics(list),
      doctors:uniqueDoctors(list).size,
      highCancelled:list.filter(r=>r.status==='Cancelada'&&r.total>=HIGH_VALUE_THRESHOLD).length
    }))
    .sort((a,b)=>b.m.estimate-a.m.estimate);

  $('#kamTable').innerHTML=kams.length
    ?kams.map((x,i)=>`<tr class="clickable" data-kam="${esc(x.name)}"><td>${i+1}</td><td><strong>${esc(x.name)}</strong></td><td>${x.m.count}</td><td class="number">${mxn.format(x.m.total)}</td><td class="number">${mxn.format(x.m.closed)}</td><td class="number">${mxn.format(x.m.estimate)}</td><td>${(x.m.conversion*100).toFixed(0)}%</td><td class="number">${mxn.format(x.m.open)}</td><td>${x.m.risk}</td><td>${x.m.cancelled}</td><td>${x.doctors}</td><td>${x.highCancelled}</td></tr>`).join('')
...
function csvGoals(file){
  ...
  reader.onload=()=>{
    const data=reader.result
      ...
      .map(l=>l.split(','))
      .filter(x=>x.length>=2)
      .map(x=>({kam:x[0].replace(/^"|"$/g,'').trim(),value:Number(x[1].replace(/[^\d.]/g,''))}))
...
function exportRow(r){
  return {
    ...
    'Nivel de riesgo':risk(r),
    Aseguradora:r.insurer,
    Sede:r.site,
    'Estatus aplicación':r.status2
  };
}
