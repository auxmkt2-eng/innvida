...
import { getFirestore, collection, doc, onSnapshot, updateDoc } from "https://www.gstatic.com/firebasejs/11.0.0/firebase-firestore.js";
...
function uniqueDoctors(list){
  ...
  list.forEach(r=>{const id=doctorIdentity(r.doctor);if(id&&!doctors.has(id))doctors.set(id,String(r.doctor).trim())});
  return doctors;
}

function normalizedReason(reason){
  const label=String(reason||"").trim();
  return {key:label?label.toLocaleLowerCase("es-MX"):"__missing__",label:label||"No reason recorded."};
}

function cancellationReasonSummary(list){
  const cancelled=list.filter(r=>r.status==="Cancelada"),groups=new Map();
  cancelled.forEach(r=>{
    const reason=normalizedReason(r.reason),entry=groups.get(reason.key)||{label:reason.label,count:0};
    entry.count++;
    groups.set(reason.key,entry);
  });
  const missing=groups.get("__missing__")?.count||0;
  const common=[...groups.entries()].filter(([key])=>key!=="__missing__").map(([,entry])=>entry).sort((a,b)=>b.count-a.count||a.label.localeCompare(b.label,"es")).at(0);
  return {count:cancelled.length,missing,common};
}

function scopedRowsForDoctorHistory(){
  const brand=$('#brandFilter').value,kam=$('#kamFilter').value,site=$('#siteFilter').value;
  return rows.filter(r=>(!brand||r.brand===brand)&&(!kam||r.kam===kam)&&(!site||r.site===site));
}

function newDoctors(){
  const from=$('#fromDate').value,to=$('#toDate').value,history=scopedRowsForDoctorHistory(),firstQuote=new Map();
  history.forEach(r=>{
    const id=doctorIdentity(r.doctor),date=validDate(r.date);
    if(!id||!date)return;
    const current=firstQuote.get(id);
    if(!current||date<current.date)firstQuote.set(id,{date,name:String(r.doctor).trim()});
  });
  ...
  }).map(([id,name])=>({id,name,firstDate:firstQuote.get(id).date})).sort((a,b)=>a.name.localeCompare(b.name,"es"));
}

function filterRows(){
  const from=$("#fromDate").value,to=$("#toDate").value,brand=$("#brandFilter").value,kam=$("#kamFilter").value,doctor=norm($("#doctorFilter").value),status=$("#statusFilter").value,insurer=norm($("#insurerFilter").value),site=$("#siteFilter").value;
  filtered=rows.filter(r=>(!from||r.date>=from)&&(!to||r.date<=to)&&(!brand||r.brand===brand)&&(!kam||r.kam===kam)&&(!doctor||norm(r.doctor).includes(doctor))&&(!status||r.status===status)&&(!insurer||norm(r.insurer).includes(insurer))&&(!site||r.site===site));
  syncGoalPeriod();
  render();
}

function metrics(list){
  const active=list.filter(r=>!isLost(r)),closed=list.filter(isClosed),open=list.filter(r=>!isClosed(r)&&!isLost(r)),cancelled=list.filter(r=>r.status==="Cancelada"),highValueCancelled=cancelled.filter(r=>r.total>=HIGH_VALUE_THRESHOLD);
  return {total:sum(list,r=>r.total),closed:sum(closed,r=>r.total),open:sum(open,r=>r.total),cancelled:cancelled.length,cancelledValue:sum(cancelled,r=>r.total),highValueCancelled:highValueCancelled.length,highValueCancelledValue:sum(highValueCancelled,r=>r.total),estimate:sum(active,estimated),count:list.length,conversion:list.length?closed.length/list.length:0,ticket:list.length?sum(list,r=>r.total)/list.length:0,risk:open.filter(r=>risk(r)==="high").length,noFollow:open.filter(r=>r.status==="Sin seguimiento").length,active};
}

function renderKpis(){
  const m=metrics(filtered),doctors=uniqueDoctors(filtered).size,newDoctorCount=newDoctors().length;
  const cards=[
...
function renderInsights(){
  ...
  const newDoctorList=newDoctors(),noKam=filtered.filter(r=>!r.kam),stuck=open.filter(r=>(daysOpen(r)||0)>30),cards=[];

  if(high.length)cards.push(insight('Rescate prioritario',`${high.length} caso(s) de alto riesgo sin siguiente acción`,`${high.map(r=>`${r.folio} (${mxn.format(r.total)})`).join(', ')}. Pedir al KAM fecha, responsable y siguiente paso hoy.`));
  if(cancelledHigh.length)cards.push(insight('Cancelaciones prioritarias',`${cancelledHigh.length} cancelación(es) de alto valor`,`${cancelledHigh.map(r=>`${r.folio} (${mxn.format(r.total)})`).join(', ')}. Ordenadas por monto; revisar causa y posibilidad de recuperación.`));

  if(cancellationReasons.count){
    const common=cancellationReasons.common,percentage=common?(common.count/cancellationReasons.count*100).toFixed(1):0;
    cards.push(insight('Motivo de cancelación',common?`${common.label}: ${common.count} caso(s), ${percentage}% de canceladas`:'No hay motivo documentado en las cancelaciones',`${cancellationReasons.missing} cancelación(es) sin motivo documentado. El motivo mostrado es el más frecuente tras normalizar espacios y mayúsculas.`));
  }

  if(kamRisk?.[1])cards.push(insight('Riesgo por responsable',`${kamRisk[0]} concentra ${mxn.format(kamRisk[1])} en riesgo`,`Solicitar plan de rescate y fecha de decisión por cada caso de riesgo.`));
  if(repeat)cards.push(insight('Médico recurrente',`${repeat[1][0].doctor.trim()} tiene ${repeat[1].length} cotizaciones`,`${mxn.format(sum(repeat[1],r=>r.total))} cotizado. La identidad del médico se normaliza para no duplicar variantes de nombre.`));
  if(newDoctorList.length)cards.push(insight('Médicos nuevos',`${newDoctorList.length} médico(s) único(s) nuevo(s)`,newDoctorList.slice(0,5).map(d=>d.name).join(', ')+(newDoctorList.length>5?'…':'')+'. Se usa la primera cotización del historial cargado con los filtros de marca, KAM y sede.'));
  if(noKam.length)cards.push(insight('Asignación',`${noKam.length} cotización(es) sin KAM asignado`,`Asignar propietario hoy: estas oportunidades no tienen responsable operativo.`));
  if(stuck.length)cards.push(insight('Antigüedad',`${stuck.length} caso(s) abiertos más de 30 días`,`Revisar viabilidad: rescatar, actualizar estatus o cerrar aprendizaje.`));

  $('#priorityCount').textContent=`${cards.length} prioridades`;
  $('#insights').innerHTML=cards.length?cards.join(''):insight('Sin señal crítica','No hay prioridades bajo los filtros actuales','Al conectar datos aparecerán hallazgos accionables; ningún dato se ha inventado.');
}
...
async function saveCase(id,brand){
  const button=document.querySelector(`[data-save="${CSS.escape(id)}"][data-brand="${brand}"]`),inputs=[...document.querySelectorAll(`[data-id="${CSS.escape(id)}"][data-brand="${brand}"]`)],changes=Object.fromEntries(inputs.map(x=>[x.dataset.field,x.value])),r=rows.find(x=>x.id===id&&x.brand===brand);
  if(!r||!dbs[brand]){showToast('Firebase aún no está listo para guardar.','error');return}
  if(changes.status==='Cerrada / aceptada'&&!changes.closeDate)changes.closeDate=isoToday();
  button.disabled=true;
  button.textContent='Guardando…';
  try{
    await updateDoc(doc(dbs[brand],r.collection,id),{status1:changes.status,motivo:changes.reason.trim(),fechaCierre:changes.closeDate,proximaAccion:changes.nextAction.trim()});
...
function fillFilters(){
  const add=(id,vals)=>{
    ...
    [...new Set(vals.filter(Boolean))].sort().forEach(v=>{if(![...el.options].some(o=>o.value===v))el.add(new Option(v,v))});
    el.value=current;
  };
...
async function initFirebase(){
  try{
    ...
    for(const [key,config] of Object.entries(firebaseProjects)){
      ...
      const brand=key.toUpperCase(),db=getFirestore(initializeApp(config,`${key}CommandCenter`));
      dbs[brand]=db;
...
function csvGoals(file){
  ...
  reader.onload=()=>{
    const data=reader.result.split(/\r?\n/).slice(1).map(l=>l.split(',')).filter(x=>x.length>=2).map(x=>({kam:x[0].replace(/^"|"$/g,'').trim(),value:Number(x[1].replace(/[^\d.]/g,''))})).filter(x=>x.kam&&x.value>=0);
    goals=data;
    saveGoals();
    renderGoals();
...
function safeSheet(s){return (s||'Sin KAM').replace(/[\\/*?:\[\]]/g,'').slice(0,31)||'Sin KAM'}
function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}
function escAttr(s){return esc(s).replace(/`/g,'&#096;')}

document.querySelectorAll('.filters input,.filters select').forEach(e=>e.addEventListener('input',filterRows));
$('#clearFilters').onclick=()=>{document.querySelectorAll('.filters input').forEach(i=>i.value='');document.querySelectorAll('.filters select').forEach(i=>i.value='');filterRows()};
$('#exportBtn').onclick=exportCEO;
$('#addGoal').onclick=()=>$('#goalDialog').showModal();
$('#saveGoal').onclick=()=>{
  const kam=$('#goalKam').value.trim(),value=Number($('#goalValue').value);
